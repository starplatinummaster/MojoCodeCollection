...
fn main():
    ...
    var X_AND: Array2D = Array2D(8, 3)
    var Y_AND: Array = Array(8)
    ...
    for af in range(3):
        print("\nTraining AND gate with", get_activation_name(af), "activation function:")
        var network = NeuralNetwork(af)
        network.fit(X_AND, Y_AND, 0.1, 5000)

        print("\nPredictions for AND gate:")
        for i in range(8):
            var result = network.feed_forward(X_AND[i, 0], X_AND[i, 1], X_AND[i, 2])
            print(X_AND[i, 0].__int__(), X_AND[i, 1].__int__(), X_AND[i, 2].__int__(), (result[0] > 0.5).__int__())
